...
describe('#whoiser.domain()', () => {
  describe('basic domain WHOIS', () => {
    it('returns WHOIS for "blog.google"', async () => {
    ...
    it('returns WHOIS for "google.co.uk"', async () => {
    ...
    it('returns WHOIS for "cloudflare.com" from "whois.cloudflare.com" server (host option)', async () => {
      const whois = await whoiser.domain('cloudflare.com', { host: 'whois.cloudflare.com' })
    ...
    it('returns WHOIS for "laye.red" from top-level whois server (follow option)', async () => {
      const whois = await whoiser.domain('laye.red', { follow: 1 })
      assert.equal(Object.values(whois).length, 1, 'Has less or more than 1 WHOIS result')
  ...
  describe('domain WHOIS parsing (handles multi-line whois, different labels and more)', () => {
    ...
    it('should return WHOIS for "de" when domain includes an umlaut', async () => {
    ...
    it('returns WHOIS for "netflix.io" with correct registrar WHOIS server', async () => {
      const whois = await whoiser.domain('netflix.io', { follow: 1 })
      assert.equal(whois['whois.nic.io']['Registrar WHOIS Server'], 'whois.markmonitor.com', 'Parsing error for WHOIS server')
    })

    it('returns WHOIS for "goo.gl" with correct registrar WHOIS server', async () => {
      const whois = await whoiser.domain('goo.gl', { follow: 1 })
      assert.equal(whois['whois.nic.gl']['Registrar WHOIS Server'], 'whois.markmonitor.com', 'Parsing error for WHOIS server')
    })

    it('returns WHOIS for "google.eu" when whois data is "label:_EOL_value"', async () => {
      const whois = await whoiser.domain('google.eu', { follow: 1 })
    ...
    it('returns WHOIS for "mañana.com" - IDN', async () => {
      const whois = await whoiser.domain('mañana.com')
    ...
    it('returns WHOIS for "XN--MAANA-PTA.COM" - IDN', async () => {
      const whois = await whoiser.domain('XN--MAANA-PTA.COM')
      assert.equal(whois['whois.verisign-grs.com']['Domain Name'], 'XN--MAANA-PTA.COM', 'Domain name doesn\'t match')
      assert.equal(whois['whois.verisign-grs.com']['Registry Domain ID'], '123697069_DOMAIN_COM-VRSN', 'Domain ID doesn\'t match')
    })

    it('returns WHOIS for "google.nl"', async () => {
      const whois = await whoiser.domain('google.nl')
    ...
    it('returns WHOIS for "jprs.jp"', async () => {
      const whois = await whoiser.domain('jprs.jp')
    ...
    it('returns WHOIS for "ownit.nyc"', async () => {
      const whois = await whoiser.domain('ownit.nyc')
    ...
    it('returns WHOIS for "nic.ua" with fieldsfor all type of contacts', async () => {
      const whois = await whoiser.domain('nic.ua')
    ...
    it('returns WHOIS for "google.it"', async () => {
