import { isDomain, splitStringBy } from './utils'

export function parseSimpleWhois(whois: string) {
  const data: any = {}
  const text: string[] = []
  ...
  whois.split('\n').forEach((line) => {
    ...
    if (line.startsWith('%') || line.startsWith('#')) {
      ...
      if (line) {
        if (line.includes(':')) {
          const [label, value] = splitStringBy(line, line.indexOf(':')).map((info: string) => info.trim())
  ...
  groups
    ...
    .forEach((group) => {
      ...
      let isGroup: boolean | string = false
      ...
      if (isGroup === 'contact') {
        ...
        for (const key in group) {
          const label = renameLabels[key as keyof typeof renameLabels] || key
...
export function parseDomainWhois(domain: string, whois: string, ignorePrivacy: boolean) {
  ...
  data['Name Server'] = data['Name Server']
    .map((nameServer: string) => nameServer.split(' '))
    .flat()
    .filter(isDomain)
...
function handleDotUa(lines: string[]) {
...
function handleDotIt(lines: string[]) {
  let section = ''
  const replacement = []
...
// Fix "label: \n value" format
function handleMultiLines(lines: string[]) {
  lines.forEach((line, index) => {
...
function handleJpLines(lines: any[]) {
  const ret: any[] = []
